// Common JS utilities for Free Degree Library Portal
const API_BASE_URL = '';

const getProxiedUrl = (endpoint, params = {}) => {
    const proxiedParams = new URLSearchParams(params || {});
    proxiedParams.set('endpoint', endpoint);
    proxiedParams.set('v', Date.now().toString());
    proxiedParams.set('_t', Date.now().toString());
    return `${API_BASE_URL}/api/proxy.php?${proxiedParams.toString()}`;
};

const safeJsonParse = async (response) => {
    const contentType = response.headers.get("content-type");
    if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Unable to load materials. Hosting provider temporarily blocked the request. Please try again later.");
    }

    const text = await response.text();
    if (
        text.includes('InfinityFree') ||
        text.includes('Browser Security Check') ||
        text.trim().startsWith('<')
    ) {
        throw new Error("Unable to load materials. Hosting provider temporarily blocked the request. Please try again later.");
    }

    try {
        const data = JSON.parse(text);
        if (data && data.status === 'error') {
            throw new Error(data.message || "An error occurred on the server.");
        }
        return data;
    } catch (e) {
        if (e.message !== "Unable to load materials. Hosting provider temporarily blocked the request. Please try again later." && !text.trim().startsWith('<') && e instanceof SyntaxError === false) {
            throw e; // re-throw if it was our explicit error
        }
        throw new Error("Unable to load materials. Hosting provider temporarily blocked the request. Please try again later.");
    }
};

const getSemesterFromTags = (tags) => {
    if (!tags) return 'N/A';
    const parts = tags.split(',').map(t => t.trim());
    const semPart = parts.find(p => p.toLowerCase().includes('sem'));
    return semPart ? semPart : 'N/A';
};

const formatDate = (dateString) => {
    const date = new Date(dateString);
    if (isNaN(date.getTime())) return dateString || '';
    const day = String(date.getDate()).padStart(2, '0');
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const year = date.getFullYear();
    return `${day}/${month}/${year}`;
};

const isViewable = (fileType) => {
    return fileType.startsWith('image/') || fileType === 'application/pdf';
};

// Volatile runtime-only userId (caching/persistence explicitly disabled)
const userId = `user_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;

const showToast = (message) => {
    let container = document.getElementById('toastContainer');
    if (!container) {
        container = document.createElement('div');
        container.id = 'toastContainer';
        container.className = 'toast-container';
        document.body.appendChild(container);
    }
    const toast = document.createElement('div');
    toast.className = 'toast';
    toast.innerHTML = `<span>💾</span> <span>${message}</span>`;
    container.appendChild(toast);

    setTimeout(() => {
        toast.classList.add('show');
    }, 10);

    setTimeout(() => {
        toast.classList.remove('show');
        setTimeout(() => {
            toast.remove();
        }, 300);
    }, 3000);
};

let pendingDownloadUrlOrId = null;
let pendingDownloadFilename = null;

const proceedWithDownload = () => {
    if (!pendingDownloadUrlOrId) return;

    showToast(`Downloading...`);
    const params = {};
    if (/^\d+$/.test(pendingDownloadUrlOrId)) {
        params.id = pendingDownloadUrlOrId;
    } else {
        params.path = pendingDownloadUrlOrId;
    }
    window.location.href = getProxiedUrl('download.php', params);

    pendingDownloadUrlOrId = null;
    pendingDownloadFilename = null;
};

const showVideoAdAndDownload = (urlOrId, filename) => {
    pendingDownloadUrlOrId = urlOrId;
    pendingDownloadFilename = filename;

    // Start the download immediately
    proceedWithDownload();

    let modal = document.getElementById('videoAdModal');
    modal.style.display = 'flex';

    const video = document.getElementById('adVideoPlayer');
    const skipBtn = document.getElementById('skipAdBtn');
    const progressBar = document.getElementById('adProgressBar');

    // Reset state
    skipBtn.disabled = true;
    skipBtn.style.backgroundColor = '#333';
    skipBtn.style.color = '#888';
    skipBtn.style.cursor = 'not-allowed';
    skipBtn.innerText = 'Skip Ad in 5s';
    progressBar.style.width = '0%';
    video.currentTime = 0;

    let skipEnabled = false;

    const finishAd = () => {
        video.pause();
        modal.style.display = 'none';
    };

    video.ontimeupdate = () => {
        if (video.duration) {
            const progress = (video.currentTime / video.duration) * 100;
            progressBar.style.width = `${progress}%`;
        }

        if (!skipEnabled) {
            const secondsPlayed = Math.floor(video.currentTime);
            const secondsLeft = Math.max(0, 5 - secondsPlayed);

            if (secondsLeft > 0) {
                skipBtn.innerText = `Skip Ad in ${secondsLeft}s`;
            } else {
                skipEnabled = true;
                skipBtn.innerText = 'Skip Ad';
                skipBtn.disabled = false;
                skipBtn.style.backgroundColor = '#F97316';
                skipBtn.style.color = '#FFFFFF';
                skipBtn.style.cursor = 'pointer';
            }
        }
    };

    video.onended = () => {
        finishAd();
    };

    skipBtn.onclick = () => {
        if (!skipBtn.disabled) {
            finishAd();
        }
    };

    // start playback immediately
    video.play().catch(e => {
        console.error("Autoplay failed:", e);
        // If autoplay fails, skip to download
        finishAd();
    });
};

const downloadFile = (urlOrId, filename) => {
    showVideoAdAndDownload(urlOrId, filename);
};

const shareContent = (title, text, path) => {
    const url = `${window.location.origin}${path}`;
    if (navigator.share) {
        navigator.share({
            title: title,
            text: text,
            url: url
        }).catch(err => console.error("Error sharing:", err));
    } else {
        navigator.clipboard.writeText(url).then(() => {
            showToast("Share link copied to clipboard!");
        }).catch(err => {
            alert("Share Link: " + url);
        });
    }
};

// Sidebar drawer functions
const openSidebar = () => {
    const sidebar = document.getElementById('sidebarMenu');
    const overlay = document.getElementById('sidebarOverlay');
    if (sidebar && overlay) {
        sidebar.classList.add('open');
        overlay.style.display = 'block';
    }
    const flagImg = document.getElementById('indFlagImg');
    if (flagImg) flagImg.style.display = 'none';
};

const closeSidebar = () => {
    const sidebar = document.getElementById('sidebarMenu');
    const overlay = document.getElementById('sidebarOverlay');
    if (sidebar && overlay) {
        sidebar.classList.remove('open');
        overlay.style.display = 'none';
    }
    const flagImg = document.getElementById('indFlagImg');
    if (flagImg) flagImg.style.display = 'block';
};

const toggleMenuDropdown = (element) => {
    const content = element.nextElementSibling;
    const arrow = element.querySelector('.arrow');
    if (content.style.display === 'none' || content.style.display === '') {
        content.style.display = 'block';
        arrow.textContent = '▴';
    } else {
        content.style.display = 'none';
        arrow.textContent = '▾';
    }
};

const alertAbout = (e) => {
    if (e) e.preventDefault();
    window.location.href = 'about.html';
};

const closeAboutModal = () => { };

const alertBugReport = (e) => {
    if (e) e.preventDefault();
    window.open("https://forms.gle/Hfaau5ZoWdGNkpdH7", "_blank");
};

const alertAdminLogin = (e) => {
    if (e) e.preventDefault();
    window.location.href = "admin.html";
};

// Check for ?search=Query parameter or window.INITIAL_ROUTE on load (makes clean URLs work)
window.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    let searchVal = urlParams.get('search');

    // Fallback to window.INITIAL_ROUTE if search param isn't present
    if (!searchVal && typeof window.INITIAL_ROUTE !== 'undefined' && window.INITIAL_ROUTE) {
        // e.g., "bca/1st-semester" -> "bca 1st semester"
        searchVal = window.INITIAL_ROUTE.replace(/\//g, ' ').replace(/-/g, ' ');
    }

    const searchInput = document.getElementById('searchQuery');
    if (searchVal && searchInput) {
        searchInput.value = searchVal;
        // Wait a brief moment to ensure fetch is completed or running, then trigger event
        setTimeout(() => {
            searchInput.dispatchEvent(new Event('input'));
        }, 300);
    }
});

// Global regex parser to detect URLs, emails, and tel numbers and make them clickable
const linkify = (text) => {
    if (!text) return '';
    let escaped = text
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
        .replace(/'/g, "&#039;");

    const urlRegex = /(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
    const emailRegex = /(\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b)/ig;
    const telRegex = /(\b\+?[0-9]{10,12}\b)/g;

    escaped = escaped.replace(urlRegex, (url) => {
        let displayUrl = url;
        if (displayUrl.length > 30) {
            displayUrl = displayUrl.substring(0, 27) + '...';
        }
        return `<a href="${url}" target="_blank" rel="noopener noreferrer">${displayUrl}</a>`;
    });
    escaped = escaped.replace(emailRegex, '<a href="mailto:$1">$1</a>');
    escaped = escaped.replace(telRegex, '<a href="tel:$1">$1</a>');

    return escaped;
};

// Load Scrolling Marquee Updates
const loadMarquee = async () => {
    const container = document.getElementById('marqueeContainer');
    if (!container) return;

    try {
        const response = await fetch(getProxiedUrl('announcements_get.php'), {
            credentials: 'include',
            cache: 'no-store',
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
        });
        if (!response.ok) return;
        const data = await safeJsonParse(response);
        if (Array.isArray(data) && data.length > 0) {
            const update = data[0]; // Take the latest/only marquee
            const linkStart = update.link_url ? `<a href="${update.link_url}" target="_blank" style="color: #FFF; text-decoration: underline;">` : '';
            const linkEnd = update.link_url ? `</a>` : '';

            container.innerHTML = `
                <div class="marquee-container">
                  <span class="blinking-tag">NEW!</span>
                  <marquee scrollamount="4" onmouseover="this.stop();" onmouseout="this.start();">${linkStart}${linkify(update.description)}${linkEnd}</marquee>
                </div>
            `;
        }
    } catch (e) {
        console.error("Failed to load marquee:", e);
    }
};

const loadSubscriptionCount = async () => {
    try {
        const response = await fetch(getProxiedUrl('subscribe.php', { action: 'count' }), {
            credentials: 'include',
            cache: 'no-store',
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
        });
        if (!response.ok) return;
        const data = await safeJsonParse(response);
        if (data.status === 'success' && typeof data.count !== 'undefined') {
            const countElements = document.querySelectorAll('.subscribe-count');
            countElements.forEach(el => {
                el.textContent = data.count;
                el.style.display = 'inline-block';
            });
        } else {
            console.error("Failed to load subscription count:", data.message || "Unknown error");
        }
    } catch (e) {
        console.error("Failed to load subscription count:", e);
    }
};

// Internship Promo Logic
const initInternshipPromo = () => {
    // Preload Ad Video for faster playback
    const preloadVideo = document.createElement('link');
    preloadVideo.rel = 'preload';
    preloadVideo.as = 'video';
    preloadVideo.href = 'Cirravo SVEP.mp4';
    document.head.appendChild(preloadVideo);

    // Pre-inject Video Ad Modal
    if (!document.getElementById('videoAdModal')) {
        const videoAdModalHtml = `
            <div id="videoAdModal" class="retro-modal-overlay" style="z-index: 4000; display: none;">
              <div class="retro-modal-card" style="padding: 15px; max-width: 360px; max-height: 90vh; display: flex; flex-direction: column; background-color: #000; border-color: #F97316; margin: 0 auto;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                  <span style="color: #F97316; font-family: 'Share Tech Mono', monospace; font-size: 14px;">Sponsored</span>
                  <button id="skipAdBtn" class="intern-promo-btn" style="padding: 6px 15px; font-size: 12px; background-color: #333; color: #888; cursor: not-allowed; box-shadow: none;" disabled>Skip Ad in 5s</button>
                </div>
                <video id="adVideoPlayer" src="Cirravo SVEP.mp4#t=0.001" preload="auto" style="width: 100%; max-height: 70vh; aspect-ratio: 9/16; object-fit: cover; border: 2px solid #333; background-color: #111; border-radius: 8px !important;" playsinline></video>
                <div style="width: 100%; height: 4px; background-color: #333; margin-top: 8px;">
                  <div id="adProgressBar" style="height: 100%; width: 0%; background-color: #F97316; transition: width 0.1s linear;"></div>
                </div>
                <div style="margin-top: 15px;">
                  <a href="https://internships.cirravosolutions.co.in/" class="intern-promo-btn" style="display: block; width: 100%; text-align: center; box-sizing: border-box;" target="_blank">Apply for Free</a>
                </div>
              </div>
            </div>
        `;
        document.body.insertAdjacentHTML('beforeend', videoAdModalHtml);
    }
    // Top Banner
    const banner = document.createElement('a');
    banner.href = "https://internships.cirravosolutions.co.in/";
    banner.className = "promo-top-banner";
    banner.innerHTML = "🚀 Unpaid Internship with Certificate. <strong>Apply Now →</strong>";
    document.body.insertBefore(banner, document.body.firstChild);

    // Modal
    const modalShown = sessionStorage.getItem('internPromoShown');
    if (!modalShown) {
        const modalHtml = `
            <div id="internPromoModal" class="intern-promo-overlay">
                <div class="intern-promo-card">
                    <button class="intern-promo-close" onclick="closeInternPromo()">×</button>
                    <div class="intern-promo-tag">🚀 Unpaid Remote Internship Program</div>
                    <div class="intern-promo-title">Launch Your Career<br>with<br><span class="intern-promo-highlight">Virtual Internships</span></div>
                    <p class="intern-promo-desc">Join our virtual internship program, complete practical tasks, build projects, and get your Internship Certificate & Offer Letter.</p>
                    <a href="https://internships.cirravosolutions.co.in/" class="intern-promo-btn">Apply Now →</a>
                </div>
            </div>
        `;
        document.body.insertAdjacentHTML('beforeend', modalHtml);
    }
};

const closeInternPromo = () => {
    const modal = document.getElementById('internPromoModal');
    if (modal) {
        modal.style.display = 'none';
        sessionStorage.setItem('internPromoShown', 'true');
    }
};

const initBrandAnimation = () => {
    // Typing effect removed as logo is now used.
};

window.addEventListener('DOMContentLoaded', () => {
    loadMarquee();
    loadSubscriptionCount();
    initWebNotifications();
    initBrandAnimation();
});

const closeSubscribeModal = () => {
    const modal = document.getElementById('subscribeRetroModal');
    if (modal) {
        modal.remove();
    }
};

const openSubscribeModal = (e) => {
    if (e) e.preventDefault();

    if (document.getElementById('subscribeRetroModal')) return;

    const modalOverlay = document.createElement('div');
    modalOverlay.id = 'subscribeRetroModal';
    modalOverlay.className = 'retro-modal-overlay';

    let browserStatus = ('Notification' in window && Notification.permission === 'granted') ? 'Enabled ✓' : 'Disabled';
    if ('Notification' in window && Notification.permission === 'denied') browserStatus = 'Blocked';

    modalOverlay.innerHTML = `
        <div class="retro-modal-card" style="max-height: 90vh; overflow-y: auto;">
            <div class="retro-modal-header">
                <span class="retro-modal-title">SUBSCRIBE TO ALERTS</span>
                <button class="retro-modal-close" onclick="closeSubscribeModal()">X</button>
            </div>
            <div class="retro-modal-body">
                <div style="background: #0F172A; border: 2px solid #818CF8; padding: 10px; margin-bottom: 20px;">
                    <p style="margin-top: 0; font-weight: bold; color: #F8FAFC;">1. Browser Notifications (Recommended)</p>
                    <p style="font-size: 13px; margin-bottom: 10px;">Get instant web notifications for new study materials and community posts while the site is open!</p>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 13px; font-family: 'Share Tech Mono', monospace;">Status: <strong id="lblBrowserStatus">${browserStatus}</strong></span>
                        <button id="btnBrowserPush" class="sub-btn-success" style="padding: 5px 10px; font-size: 12px;">Enable Web Notifications</button>
                    </div>
                </div>
                
                <p style="font-weight: bold;">2. Optional: Email Alerts</p>
                <p style="font-size: 13px; margin-bottom: 15px;">Get real-time email notifications for academic updates!</p>
                
                <div class="form-group">
                    <label for="subEmail">Email Address</label>
                    <input type="email" id="subEmail" placeholder="Enter your email..." style="width: 100%; box-sizing: border-box;">
                </div>

                <div class="sub-checkbox-group">
                    <label class="sub-checkbox-label">
                        <input type="checkbox" id="subAnnouncements" checked>
                        <span>📢 Official Announcements & Polls</span>
                    </label>
                    <label class="sub-checkbox-label">
                        <input type="checkbox" id="subCommunity" checked>
                        <span>💬 Community Discussions & Doubts</span>
                    </label>
                    <label class="sub-checkbox-label">
                        <input type="checkbox" id="subMaterials" checked>
                        <span>📚 Study Materials & Question Papers</span>
                    </label>
                </div>

                <div id="subStatusMsg" style="font-family: 'Share Tech Mono', monospace; font-size: 14px; margin-bottom: 15px; display: none;"></div>

                <div class="modal-actions">
                    <button id="btnSubscribeAction" class="sub-btn-success">Update Email Subscription</button>
                    <button id="btnUnsubscribeAction" class="sub-btn-danger" style="display: none;">Unsubscribe Email</button>
                </div>
            </div>
        </div>
    `;

    modalOverlay.addEventListener('click', (event) => {
        if (event.target === modalOverlay) {
            closeSubscribeModal();
        }
    });

    document.body.appendChild(modalOverlay);

    const btnBrowserPush = document.getElementById('btnBrowserPush');
    const lblBrowserStatus = document.getElementById('lblBrowserStatus');

    if (localStorage.getItem('browser_notifications_enabled') === 'true' && Notification.permission === 'granted') {
        btnBrowserPush.innerText = 'Test Notification';
    }

    btnBrowserPush.addEventListener('click', async () => {
        if (!('Notification' in window)) {
            alert('This browser does not support desktop notification');
            return;
        }
        if (Notification.permission === 'granted') {
            localStorage.setItem('browser_notifications_enabled', 'true');
            btnBrowserPush.innerText = 'Test Notification';
            lblBrowserStatus.innerText = 'Enabled ✓';
            new Notification('Test Notification', { body: 'Web notifications are working perfectly!', icon: 'logo.png' });
        } else if (Notification.permission !== 'denied') {
            const permission = await Notification.requestPermission();
            if (permission === 'granted') {
                localStorage.setItem('browser_notifications_enabled', 'true');
                btnBrowserPush.innerText = 'Test Notification';
                lblBrowserStatus.innerText = 'Enabled ✓';
                new Notification('Notifications Enabled!', { body: 'You will now receive updates when the site is open.', icon: 'logo.png' });
            }
        } else {
            alert('Notifications are blocked. Please enable them in your browser settings.');
        }
    });

    const emailInput = document.getElementById('subEmail');
    const announcementsCb = document.getElementById('subAnnouncements');
    const communityCb = document.getElementById('subCommunity');
    const materialsCb = document.getElementById('subMaterials');
    const statusMsg = document.getElementById('subStatusMsg');
    const btnSubscribe = document.getElementById('btnSubscribeAction');
    const btnUnsubscribe = document.getElementById('btnUnsubscribeAction');

    const setStatus = (msg, isError = false) => {
        statusMsg.style.display = 'block';
        statusMsg.style.color = isError ? '#FF0000' : '#10B981';
        statusMsg.innerText = msg;
    };

    const checkStatus = async () => {
        const email = emailInput.value.trim();
        if (!email || !email.includes('@')) {
            btnUnsubscribe.style.display = 'none';
            btnSubscribe.innerText = 'Subscribe Email';
            btnSubscribe.className = 'sub-btn-success';
            return;
        }

        try {
            const response = await fetch(getProxiedUrl('subscribe.php', { email }));
            if (!response.ok) return;
            const data = await safeJsonParse(response);
            if (data.status === 'success' && data.subscribed) {
                announcementsCb.checked = data.preferences.announcements;
                communityCb.checked = data.preferences.community;
                materialsCb.checked = data.preferences.materials;
                btnUnsubscribe.style.display = 'inline-block';
                btnSubscribe.innerText = 'Update Preferences';
                btnSubscribe.className = 'sub-btn-success';
                setStatus('Email found! You can update preferences or unsubscribe below.');
            } else {
                btnUnsubscribe.style.display = 'none';
                btnSubscribe.innerText = 'Subscribe Email';
                btnSubscribe.className = 'sub-btn-success';
                statusMsg.style.display = 'none';
            }
        } catch (e) {
            console.error(e);
        }
    };

    emailInput.addEventListener('input', () => {
        if (emailInput.value.includes('@') && emailInput.value.includes('.')) {
            checkStatus();
        } else {
            btnUnsubscribe.style.display = 'none';
            btnSubscribe.innerText = 'Subscribe Email';
            btnSubscribe.className = 'sub-btn-success';
            statusMsg.style.display = 'none';
        }
    });

    btnSubscribe.addEventListener('click', async () => {
        const email = emailInput.value.trim();
        if (!email) {
            setStatus('Email address is required to subscribe via email.', true);
            return;
        }
        if (!email.includes('@') || !email.includes('.')) {
            setStatus('Please enter a valid email address.', true);
            return;
        }

        setStatus('Processing request...');
        btnSubscribe.disabled = true;

        try {
            const response = await fetch(getProxiedUrl('subscribe.php'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'no-cache, no-store, must-revalidate',
                    'Pragma': 'no-cache'
                },
                body: JSON.stringify({
                    action: 'subscribe',
                    email: email,
                    notify_announcements: announcementsCb.checked ? 1 : 0,
                    notify_community: communityCb.checked ? 1 : 0,
                    notify_materials: materialsCb.checked ? 1 : 0
                }),
                credentials: 'include',
                cache: 'no-store'
            });

            const data = await safeJsonParse(response);
            if (response.ok && data.status === 'success') {
                setStatus('Email subscription updated! A confirmation email has been sent.');
                showToast('Subscription updated successfully!');
                loadSubscriptionCount();
                setTimeout(closeSubscribeModal, 2000);
            } else {
                setStatus(data.message || 'Failed to update subscription.', true);
            }
        } catch (err) {
            setStatus('Failed to update subscription. Try again later.', true);
        } finally {
            btnSubscribe.disabled = false;
        }
    });

    btnUnsubscribe.addEventListener('click', async () => {
        const email = emailInput.value.trim();
        if (!email) return;

        setStatus('Unsubscribing...');
        btnUnsubscribe.disabled = true;

        try {
            const response = await fetch(getProxiedUrl('subscribe.php'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'no-cache, no-store, must-revalidate',
                    'Pragma': 'no-cache'
                },
                body: JSON.stringify({
                    action: 'unsubscribe',
                    email: email
                }),
                credentials: 'include',
                cache: 'no-store'
            });

            const data = await safeJsonParse(response);
            if (response.ok && data.status === 'success') {
                setStatus('You have been unsubscribed successfully.');
                showToast('Unsubscribed successfully.');
                loadSubscriptionCount();
                setTimeout(closeSubscribeModal, 2000);
            } else {
                setStatus(data.message || 'Failed to unsubscribe.', true);
            }
        } catch (err) {
            setStatus('Failed to unsubscribe. Try again later.', true);
        } finally {
            btnUnsubscribe.disabled = false;
        }
    });
};

const initWebNotifications = async () => {
    if (!('Notification' in window)) return;

    const checkUpdates = async () => {
        if (Notification.permission !== 'granted') return;
        if (localStorage.getItem('browser_notifications_enabled') !== 'true') return;

        try {
            const response = await fetch(getProxiedUrl('latest_updates.php'), { cache: 'no-store' });
            if (!response.ok) return;
            const data = await safeJsonParse(response);
            if (data.status !== 'success') return;

            const storedMatId = parseInt(localStorage.getItem('latest_material_id') || '0');
            const storedPostId = parseInt(localStorage.getItem('latest_post_id') || '0');

            // If it's the very first time, just store them silently so we don't spam
            if (storedMatId === 0 && storedPostId === 0) {
                localStorage.setItem('latest_material_id', data.latest_material_id);
                localStorage.setItem('latest_post_id', data.latest_post_id);
                return;
            }

            if (data.latest_material_id > storedMatId) {
                new Notification('Free Degree Library Alert', {
                    body: 'A new study material was uploaded!',
                    icon: 'logo.png'
                });
                localStorage.setItem('latest_material_id', data.latest_material_id);
            }

            if (data.latest_post_id > storedPostId) {
                new Notification('Free Degree Library Alert', {
                    body: 'A new post in the Community Board!',
                    icon: 'logo.png'
                });
                localStorage.setItem('latest_post_id', data.latest_post_id);
            }
        } catch (e) {
            console.error('Polling error', e);
        }
    };

    // Run immediately
    checkUpdates();

    // Then poll every 30 seconds
    setInterval(checkUpdates, 30000);
};


// Donate Modal Logic



const closeHelpModal = () => {
    const modal = document.getElementById('helpModal');
    if (modal) {
        // Move the translate widget back to the body before destroying the modal
        const translateEl = document.getElementById('google_translate_element');
        if (translateEl) {
            translateEl.style.display = 'none';
            document.body.appendChild(translateEl);
        }
        modal.remove();
    }
};

let googleTranslateInjected = false;

const injectGoogleTranslate = () => {
    if (googleTranslateInjected) return;
    
    if (!document.getElementById('google_translate_element')) {
        const dummyDiv = document.createElement('div');
        dummyDiv.id = 'google_translate_element';
        dummyDiv.style.display = 'none';
        document.body.appendChild(dummyDiv);
    }

    window.googleTranslateElementInit = function() {
        new google.translate.TranslateElement({pageLanguage: 'en'}, 'google_translate_element');
    };
    const script = document.createElement('script');
    script.type = 'text/javascript';
    script.src = '//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit';
    document.body.appendChild(script);
    googleTranslateInjected = true;
};

document.addEventListener('DOMContentLoaded', () => {
    // If user has a translation preference, auto-load the script to persist it across pages
    if (document.cookie.indexOf('googtrans=') !== -1) {
        injectGoogleTranslate();
    }
});

const openHelpModal = (e) => {
    if (e) e.preventDefault();

    if (document.getElementById('helpModal')) return;

    const modalOverlay = document.createElement('div');
    modalOverlay.id = 'helpModal';
    modalOverlay.className = 'retro-modal-overlay';
    
    modalOverlay.innerHTML = `
        <div class="retro-modal-card" style="max-height: 90vh; overflow-y: auto; text-align: center; width: 350px;">
            <div class="retro-modal-header">
                <span class="retro-modal-title">HELP & SETTINGS</span>
                <button class="retro-modal-close" id="helpModalCloseBtn">X</button>
            </div>
            <div class="retro-modal-body" style="padding: 20px;">
                <p style="font-weight: bold; margin-bottom: 10px;">Select Language</p>
                <div id="translate_container" style="margin-bottom: 25px; min-height: 35px; background: white; border-radius: 4px; padding: 4px;"></div>
                
                <p style="font-weight: bold; margin-bottom: 10px;">Accessibility & Vibe</p>
                <select id="fontSelectPref" onchange="changeFontFamily(this.value)" style="width: 100%; padding: 10px; margin-bottom: 10px; border: 2px solid #1E293B; border-radius: 4px; font-family: 'Share Tech Mono', monospace; font-weight: bold; background-color: #F8FAFC; color: #1E293B; cursor: pointer;">
                    <option value="default">Retro Mono (Default)</option>
                    <option value="sans">Modern (Sans-serif)</option>
                    <option value="serif">Classic (Serif)</option>
                    <option value="cursive">Cursive (Comic)</option>
                    <option value="bold">Bold Retro</option>
                </select>
                <button onclick="toggleFontSize()" class="sub-btn-success" style="margin-bottom: 10px; width: 100%; padding: 10px; font-weight: bold; background-color: #3B82F6; color: white; border: none; border-radius: 4px; cursor: pointer;">Toggle Large Text</button>
                <button id="toggleMusicBtn" onclick="toggleBgMusic()" class="sub-btn-success" style="margin-bottom: 25px; width: 100%; padding: 10px; font-weight: bold; background-color: #8B5CF6; color: white; border: none; border-radius: 4px; cursor: pointer;">Play Lo-Fi Study Music 🎵</button>
                
                <p style="font-weight: bold; margin-bottom: 10px;">Support</p>
                <a href="mailto:cirravosolutions@gmail.com" class="sub-btn-danger" style="display: block; width: 100%; padding: 10px; font-weight: bold; text-decoration: none; box-sizing: border-box; background-color: #EF4444; color: white; border-radius: 4px;">Mail to Admin</a>
                <button onclick="document.getElementById('reportIframeContainer').style.display='block'; this.style.display='none';" class="sub-btn-success" style="display: block; width: 100%; padding: 10px; font-weight: bold; background-color: #F59E0B; color: white; border: none; border-radius: 4px; margin-top: 10px; cursor: pointer; font-family: 'Share Tech Mono', monospace;">Report Broken Link ⚠️</button>
                <div id="reportIframeContainer" style="display:none; margin-top:15px; width:100%; height:300px; border: 2px solid #1E293B; border-radius: 4px; overflow: hidden;">
                    <iframe src="https://docs.google.com/forms/d/e/1FAIpQLSeEF_6N97NeEWvuJ7mDHTRprd_RiIfD9HAfiqJtwO1g9pGVUg/viewform?embedded=true" width="100%" height="300" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>
                </div>
                <a href="about.html" class="sub-btn-success" style="display: block; width: 100%; padding: 10px; font-weight: bold; text-decoration: none; box-sizing: border-box; background-color: #10B981; color: white; border-radius: 4px; margin-top: 10px;">About Us</a>
            </div>
        </div>
    `;

    document.body.appendChild(modalOverlay);

    document.getElementById('helpModalCloseBtn').addEventListener('click', closeHelpModal);
    modalOverlay.addEventListener('click', (ev) => {
        if (ev.target === modalOverlay) closeHelpModal();
    });

    injectGoogleTranslate();
    
    setTimeout(() => {
        const savedFont = localStorage.getItem('user_font_pref') || 'default';
        const fontSelect = document.getElementById('fontSelectPref');
        if(fontSelect) fontSelect.value = savedFont;
        
        const musicBtn = document.getElementById('toggleMusicBtn');
        if(musicBtn && bgAudio && !bgAudio.paused) {
            musicBtn.innerHTML = 'Pause Music ⏸️';
        }
        
        const translateEl = document.getElementById('google_translate_element');
        if (translateEl) {
            translateEl.style.display = 'block';
            document.getElementById('translate_container').appendChild(translateEl);
        }
    }, 10);
};

const toggleFontSize = () => {
    const currentSize = document.body.style.zoom || "100%";
    if (currentSize === "100%") {
        document.body.style.zoom = "115%";
    } else {
        document.body.style.zoom = "100%";
    }
};

let bgAudio = null;
const toggleBgMusic = () => {
    const btn = document.getElementById('toggleMusicBtn');
    if (!bgAudio) {
        bgAudio = new Audio('/bg_music.mp3');
        bgAudio.loop = true;
        bgAudio.volume = 0.5;
    }
    
    if (bgAudio.paused) {
        bgAudio.play().catch(e => {
            console.error('Audio play failed:', e);
            alert("Please make sure you have 'bg_music.mp3' in the root directory!");
        });
        if(btn) btn.innerHTML = 'Pause Music ⏸️';
        localStorage.setItem('play_bg_music', 'true');
    } else {
        bgAudio.pause();
        if(btn) btn.innerHTML = 'Play Lo-Fi Study Music 🎵';
        localStorage.setItem('play_bg_music', 'false');
    }
};

const changeFontFamily = (val) => {
    let fontStr = '';
    if (val === 'default') {
        fontStr = "'Share Tech Mono', monospace, Courier New, sans-serif";
    } else if (val === 'sans') {
        fontStr = "Arial, Helvetica, sans-serif";
    } else if (val === 'serif') {
        fontStr = "'Times New Roman', Times, serif";
    } else if (val === 'cursive') {
        fontStr = "'Comic Sans MS', cursive, sans-serif";
    } else if (val === 'bold') {
        fontStr = "'Share Tech Mono', monospace, Courier New, sans-serif";
        document.body.style.fontWeight = 'bold';
    }
    
    if (val !== 'bold') {
        document.body.style.fontWeight = 'normal';
    }
    document.body.style.fontFamily = fontStr;
    localStorage.setItem('user_font_pref', val);
};

window.addEventListener("DOMContentLoaded", () => {
    const savedFont = localStorage.getItem('user_font_pref');
    if(savedFont) changeFontFamily(savedFont);
    
    if (localStorage.getItem('play_bg_music') === 'true') {
        bgAudio = new Audio('/bg_music.mp3');
        bgAudio.loop = true;
        bgAudio.volume = 0.5;
        document.body.addEventListener('click', () => {
            if(localStorage.getItem('play_bg_music') === 'true' && bgAudio && bgAudio.paused) {
                bgAudio.play().catch(()=>{});
            }
        }, {once: true});
    }
});

// Load Lucide Icons
const lucideScript = document.createElement('script');
lucideScript.src = 'https://unpkg.com/lucide@latest';
lucideScript.onload = () => lucide.createIcons();
document.head.appendChild(lucideScript);
